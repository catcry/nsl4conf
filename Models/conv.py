...
act_funct = 'elu'#keras.layers.LeakyReLU(alpha=0.3)#'relu'
auxi_act_funct = 'elu' #keras.layers.LeakyReLU(alpha=0.3)  #for other dense layers
...
#        model_c.add(Dense(100,activation = auxi_act_funct,input_shape = (None,Xtrain.shape[2])))
#>>>>>  Layer 1 : Conv Layer---------------------------------------------------       
...
#>>>>>   Layer 4 : output layer -----------------------------------------------
        ...
        callback = tf.keras.callbacks.EarlyStopping(monitor='val_loss',\
                                                        ...
                                                        patience = 3,\
                                                        mode  = 'min')
        history_c = model_c.fit (Xtrain,Ytrain_1hot, \
                     epochs = num_epochs, \
                     batch_size = batch, \
...
#        
        ...
        #ts_result = time.time()
        yh_Xtrain_c = model_c.predict(Xtrain)
        ...
        yh_valid_c = model_c.predict(Xvalid)
        ...
        ac_test_c_mean_intime[i+1] = (sum(ac_valid_c_mean_intime) + ac_valid_c[i]) / (i+1)
           #______________________________________________________________________ 
        yh_test_c = model_c.predict(Xtest)
